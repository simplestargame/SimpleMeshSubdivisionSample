...
namespace SimplestarGame
{
    ...
    public struct CountChunkVertexJob : IJobParallelFor
    {
        ...
        {
            var offsetCube = (x + chunkOffset.x) * this.dataEdgeCubeCount * this.dataEdgeCubeCount 
                + (y + chunkOffset.y) * this.dataEdgeCubeCount 
                + (z + chunkOffset.z);
        ...
        public void Execute(int index)
        {
            ...
            if (this.GetVoxelValue(ref voxelData, x, y, z) == 255)
            {
                for (int c = 0; c < this.vertexCounts.Length; c++)
                {
                    ...
                    if (c == CAWFile.PLUS_X)
                    {
                        if (x + chunkOffset.x + 1 != this.dataEdgeCubeCount)
                        {
                    ...
                    {
                        if (y + chunkOffset.y + 1 != this.dataEdgeCubeCount)
                        {
                    ...
                    {
                        if (z + chunkOffset.z + 1 != this.dataEdgeCubeCount)
                        {
